...
export default function App() {
 ...
 // --- SENSOR BACA BRANKAS JALUR VVIP SUPABASE ---
  useEffect(() => {
    const cekSesiSupabase = async () => {
      try {
        // 1. Tanya langsung ke Direktur Utama (Supabase)
        const { data: { session }, error } = await supabase.auth.getSession();

        if (error) {
          console.error("Gagal cek sesi:", error.message);
          return; // Kalau error, berhenti di sini
        }

        // 2. Kalau ada izin masuk dari Supabase
        if (session && session.user) {
          const emailUser = session.user.email;

          // 3. Tarik data profilnya
          const { data: userData } = await supabase.from('users').select('*').eq('email', emailUser).single();
          ...
          if (userData) {
            ...
            setEmail(emailUser);
            setIsLoggedIn(true); // BUKA GERBANG!
            fetchAllData();
            fetchMyOrders();
          }
        }
    ...
    cekSesiSupabase(); // Jalankan!

    // --- (Opsional tapi sangat disarankan) ---
    // Pasang "Mata-mata" biar kalau status login berubah di tempat lain, aplikasinya otomatis tahu.
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session) {
         // (Bos bisa tambahkan logika penarikan data lagi di sini jika perlu, 
         // tapi untuk sekarang kita biarkan kosong agar tidak double-fetch)
    ...
    return () => {
      authListener.subscription.unsubscribe(); // Bersihkan mata-mata saat komponen ditutup
    };
    // ----------------------------------------
...
setIsLoggedIn(true);
        ...
        // --- TAMBAHAN KODE BRANKAS BOS ELVAN ---
  ...
  if (view === 'chatRoom' && activeChat) {
    ...
    const channel = supabase
      ...
      // --- RADAR PENYADAP TYPING (BROADCAST) ---
          .on(
            'broadcast',
            { event: 'typing' },
            (payload) => {
              // Cek apakah sinyalnya dari ruangan ini dan BUKAN dari diri kita sendiri
              if (payload.payload.roomId === getRoomId() && payload.payload.userId !== currentUser.id) {
                setIsTyping(true); // Nyalakan animasi typing!
                
                // Timer pintar (Biar hilang otomatis kalau dia berhenti ngetik 2 detik)
  ...
  if (!isLoaded) return (
    ...
    <div className={`min-h-screen bg-slate-950 overscroll-none flex flex-col items-center justify-center relative overflow-hidden ${isDarkMode ? '' : 'light-mode'}`}>
  ...
  if (!isLoggedIn) return (
    <div className={`min-h-screen bg-slate-950 overscroll-none flex flex-col items-center justify-center p-6 text-slate-200 relative overflow-hidden ${isDarkMode ? '' : 'light-mode'}`}>
...
<div className="flex flex-row items-center justify-center gap-2 mb-2 mt-2">
  <img src="/icon-192.png" alt="Logo" className="w-10 h-10 rounded-xl shadow-[0_0_15px_rgba(37,99,235,0.6)]" />
  <h1 className="text-2xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 tracking-tight">
    ElvanTechnoShop
  </h1>
</div>
  ...
  <div className="flex w-full mt-2">
    <div className="bg-slate-800 rounded-2xl rounded-tl-sm px-4 py-2.5 shadow-md flex items-center gap-1.5 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></div>
                ...
                <div className="p-4 bg-slate-900 border-t border-slate-800 pb-8"><form onSubmit={handleSendMessage} className="flex gap-2"><input type="text" value={newMsgText} onChange={(e) => {
  setNewMsgText(e.target.value);
  if (e.target.value.trim() !== "") {
    const activeChannel = supabase.getChannels().find(c => c.topic === 'realtime:realtime-chat');
    if (activeChannel) {
      activeChannel.send({
        type: 'broadcast',
        event: 'typing',
        payload: { roomId: getRoomId(), userId: currentUser.id }
      });
    }
    }
  }
} placeholder="Ketik pesan..." className="flex-1 bg-slate-800 rounded-full px-5 py-3 text-sm text-white outline-none focus:border focus:border-blue-500 transition-all" /><button type="submit" disabled={!newMsgText.trim()} className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white disabled:opacity-50 shrink-0 shadow-lg shadow-blue-600/30"><Send size={18} className="-ml-0.5 mt-0.5" /></button></form></div>
             </div>
          )}
