...
export default function App() {
 ...
 // --- SENSOR BACA BRANKAS BOS ELVAN (VERSI ANTI-CRASH) ---
  useEffect(() => {
    const cekBrankas = async () => {
      const statusBrankas = localStorage.getItem("isLoggedIn");
      const emailBrankas = localStorage.getItem("userEmail");

      if (statusBrankas === "true" && emailBrankas) {
        try {
          // 1. Tarik ulang data profil dari Supabase pakai email di brankas!
          const { data: userData } = await supabase.from('users').select('*').eq('email', emailBrankas).single();
          
          if (userData) {
            // 2. Isi semua data penting dulu sebelum buka gerbang
            setCurrentUser(userData);
            setEditData(userData);
            setEmail(emailBrankas);
            
            // 3. Baru gerbangnya dibuka!
            setIsLoggedIn(true);
            
            // 4. Panggil produk dan pesanan
            fetchAllData();
            fetchMyOrders();
          }
        } catch (err) {
          console.error("Gagal baca brankas:", err);
          // Kalau gagal, hapus memori brankas biar nggak error terus
