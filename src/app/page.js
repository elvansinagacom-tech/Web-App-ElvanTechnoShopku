...
// --- SENSOR BACA BRANKAS JALUR VVIP (VERSI BERSIH & AMAN) ---
  useEffect(() => {
    const cekSesiSupabase = async () => {
      try {
        ...
        if (error) return; // Kalau error, diam aja nggak usah panik

        // 2. Kalau ada tiket masuk
        if (session && session.user) {
          const emailUser = session.user.email;

          // 3. Tarik data profil Bos Elvan
          const { data: userData } = await supabase.from('users').select('*').eq('email', emailUser).single();

          if (userData) {
            // 4. Set semua state sebelum buka pintu
    ...
    cekSesiSupabase(); // Nyalakan mesinnya!
  }, []);
  // ------------------------------------------------------------
  // CSS Custom
  useEffect(() => {
