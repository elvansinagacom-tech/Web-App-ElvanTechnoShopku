...
// --- SENSOR BACA COOKIES (ANTI WIPE APK) BOS ELVAN ---
  useEffect(() => {
    const cekBrankasCookie = async () => {
      // 1. Fungsi rahasia untuk membaca Cookie
      const getCookie = (nama) => {
        const value = `; ${document.cookie}`;
        const parts = value.split(`; ${nama}=`);
        if (parts.length === 2) return decodeURIComponent(parts.pop().split(';').shift());
        return null;
      };

      // 2. Ambil data dari Cookie
      const statusCookie = getCookie("isLoggedIn");
      const emailCookie = getCookie("userEmail");

      // 3. Kalau Cookie-nya ada, Dobrak Pintunya!
      if (statusCookie === "true" && emailCookie) {
        try {
          ...
          if (userData) {
            ...
            setIsLoggedIn(true); // GERBANG TERBUKA!
            
            fetchAllData();
            fetchMyOrders();
          }
        } catch (err) {
          console.error("Aman, cuma gangguan sinyal:", err);
        }
      }
    };

    cekBrankasCookie(); // Gas pol!
  }, []);
  // ----------------------------------------------------
  // CSS Custom
  useEffect(() => {
...
setIsLoggedIn(true);
        ...
        // --- SISTEM KUE / COOKIE (ANTI WIPE APK) ---
        document.cookie = `isLoggedIn=true; max-age=2592000; path=/`;
        document.cookie = `userEmail=${email}; max-age=2592000; path=/`;
        // -------------------------------------------
        fetchAllData();
        fetchMyOrders();
