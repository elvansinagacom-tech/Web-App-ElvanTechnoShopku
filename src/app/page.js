...
export default function App() {
  ...
  // --- SENSOR BACA BRANKAS BOS ELVAN ---
  useEffect(() => {
    // Cek isi brankas saat aplikasi pertama kali dibuka
    const statusBrankas = localStorage.getItem("isLoggedIn");
    const emailBrankas = localStorage.getItem("userEmail");

// Kalau di brankas ada tanda "true" (berarti udah pernah login)
    if (statusBrankas === "true") {
      setIsLoggedIn(true); // Langsung buka gerbang ke halaman utama!
      
      if (emailBrankas) {
        setEmail(emailBrankas); // Ingat juga emailnya biar nggak kosong
      }
