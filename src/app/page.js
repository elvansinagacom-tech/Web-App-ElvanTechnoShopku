...
// --- SENSOR BACA BRANKAS MANUAL (VERSI BAJA TITANIUM) ---
  useEffect(() => {
    const cekBrankasLokal = async () => {
      // 1. Cek Loker HP Bos
      const statusLokal = localStorage.getItem("isLoggedIn");
      const emailLokal = localStorage.getItem("userEmail");

      // 2. Kalau ada KTP Bos di loker
      if (statusLokal === "true" && emailLokal) {
        try {
          // 3. Tarik data dari TABEL CUSTOM buatan Bos Elvan
          ...
          if (userData) {
            // 4. Data ketemu! Pasang semuanya!
            setCurrentUser(userData);
            setEditData(userData);
            setEmail(emailLokal);
            
            setIsLoggedIn(true); // DOBRAK PINTUNYA!
          ...
          // JANGAN hapus brankas kalau cuma error sinyal/jaringan!
          console.error("Gagal nyambung ke Supabase, tapi brankas aman:", err);
        }
      }
    };

    cekBrankasLokal(); // Gas pol!
  }, []);
  // --------------------------------------------------------
  // CSS Custom
  useEffect(() => {
...
setIsLoggedIn(true);
  ...
  const fetchChatMessages = async () => {
     ...
     setIsFetchingChat(true); // Mulai loading!
     try {
        let clearedAt = null;
    ...
    setIsFetchingChat(false); // Matikan loading!
  }
  }
...
const openChatRoom = (chatType, user = null) => {
     ...
     setView('chatRoom');
     setChatMessages([]);
     setLoading(true); // Trik 1: Nyalakan animasi loading!
  }
          ...
          .on(
            ...
            (payload) => {
              ...
              if (payload.payload.roomId === getRoomId() && payload.payload.userId !== currentUser.id) {
                ...
                // Timer pintar (Biar hilang otomatis kalau dia berhenti ngetik 3 detik)
...
<div className="flex flex-row items-center justify-center gap-2 mb-2 mt-2">
  ...
  <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-80">
    ...
    <p className="text-xs font-bold">Memuat pesan...</p>
  </div>
